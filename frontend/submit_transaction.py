...
def create_txn(private_key_input, data, task_id):
    private_key_hex = private_key_input  # luc nay private key dang o dang hex
    ...
    address = hashlib.sha512('upload_model'.encode(
        "utf-8")).hexdigest()[0:6] + hashlib.sha512(payload['public_key'].encode('utf-8')).hexdigest()[:64]
    print(address)

    txn_header_bytes = TransactionHeader(
        family_name='upload_model',
        family_version='1.0',
        inputs=[str(address)],
        outputs=[str(address)],
...
def submit_txn(private_key, model, task_id):
    try:
        batch_list_bytes = create_txn(private_key, model, task_id)
